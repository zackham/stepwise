...
2. **Run a flow:** `stepwise run <name> --wait --var k=v` — **always run as a background process** (e.g., `run_in_background=True` in Bash tool). This gives you automatic notification on completion/failure/suspension with full JSON results while keeping your session free for conversation. Never block your session with a foreground `--wait`.
