...
def _format_compact(entries: list[dict], registry_entries: list[dict] | None = None) -> str:
    ...
    lines.extend([
        ...
        "",
        "# 4. Wait for all jobs (run in background for async notification)",
        ...
        "  `stepwise job show [--group <name>]` — list staged jobs.",
        "  `stepwise job run [--group <name>] [--max-concurrent N]` — release to pending.",
        "  `stepwise job cancel <id>` — cancel a staged/pending job.",
        "  `stepwise job rm <id>` — delete a staged job.",
        "  `stepwise wait $ID1 $ID2 ... --all` — block until all jobs complete (run in background).",
        "",
