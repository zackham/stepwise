...
def _format_compact(entries: list[dict], registry_entries: list[dict] | None = None) -> str:
    ...
    lines.extend([
        ...
        "",
        "**Async** — Run in the background, check status later.",
        "  `stepwise run <flow> --async --var k=v`",
        "  Returns immediately with `{\"job_id\": \"...\"}`. Poll with:",
        "  `stepwise status <job-id>` — check progress.",
        "  `stepwise output <job-id>` — get results when done.",
        "  Use for concurrent jobs or long-running work.",
    ...
    lines.extend([
        ...
        "",
        "**Preferred: background `--wait`**",
        "Run each job with `--wait` in a background process. You get automatic notification",
        "on completion with the full result — no polling needed. This is better than `--async`",
        "because results flow back to you without manual retrieval.",
        "",
