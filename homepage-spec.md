...
- **Particle overlay (idle):** animated dashed layer on ALL edges — `stroke: var(--accent-bright)` (#96eafe), `stroke-width: 2`, `stroke-dasharray: 6 14`, flowing animation 1.5s infinite. Gives the DAG a living, flowing feel at rest.
- **Particle overlay (running):** when execution is active, particles are shown ONLY on edges feeding currently running/executing nodes. All other edges lose their particle overlay and dim to the base stroke. This creates a visual "flow of data" effect — you can see exactly where work is happening by following the animated lines.
- **Active edge state:** edges feeding a running node get `stroke: rgba(34,211,238,0.5)`, brighter particle overlay (`stroke-opacity: 1`), `filter: drop-shadow(0 0 4px var(--accent-glow))`, active arrowhead marker (cyan).
- **Completed edge state:** edges between two completed nodes return to base stroke with no particles (work is done flowing through them).
...
**Node states:**
- `idle` → default rendering (type-colored border, full opacity)
- `running` → **high-contrast active state:** node fill brightens to `rgba(34,211,238,0.08)` (subtle cyan tint), border glows with `box-shadow: 0 0 20px <type-color>`, `stroke-width: 2.5px`, rotating dashed spinner circle overlaid (`stroke-dasharray: 8 24`, spin animation). The node should be unmistakably "the thing that's working right now."
- `completed` → border returns to type color at full opacity, `stroke-width: 1.5px` (back to normal). Fill returns to default. Timing label appears next to node (e.g. "1.3s") in `var(--text-muted)`. No checkmarks — the absence of glow/spinner is sufficient. Completed nodes look like idle nodes with a timing label.
...
3. Children enter `running` state individually (same glow/spinner as main nodes, scaled down)
4. Children complete individually with staggered timing: glow fades, returns to idle appearance. No checkmarks.
...
"Built with Stepwise" section explaining the meta-story: this entire site was generated by a Stepwise flow (`generate-homepage.flow.yaml` — the flow that built this page). The flow is browsable at `/flows/generate-website`. Link to the flow detail page.
