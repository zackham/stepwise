...
**Containment for agent isolation.** Agent steps can run inside hardware-isolated [Cloud-Hypervisor](https://www.cloudhypervisor.org/) microVMs, bounding the blast radius of autonomous sessions. The adapter (Claude Code, Codex, aloop, or any ACP-compatible agent) runs *inside* the VM — every Read/Write/Bash tool call is contained, not just the LLM call. Each VM gets only the filesystem paths, credentials, and network endpoints declared in its config — a research agent can never access a deploy agent's AWS keys because they run in separate hardware boundaries. Multi-step sessions reuse a single VM: a five-step Claude session boots one VM, not five. Verified end-to-end across all three ACP adapters by a four-tier staircase (smoke → toolbox → boundary probes → multi-step session continuity); see [Containment](docs/containment.md).
