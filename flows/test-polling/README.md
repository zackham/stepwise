...
2. Run with long delays:
   ```bash
   stepwise run flows/test-polling/FLOW.yaml \
     --var basic_delay_seconds=60 \
     --var longrun_delay_seconds=120
   ```
3. Wait for poll-basic to enter SUSPENDED state
4. Restart: `stepwise server restart`
5. Observe whether poll-basic resumes checking

## Known issue: poll watch restart gap

After a server restart, poll watch timers are **not re-scheduled**. The `_schedule_poll_watch` method (`engine.py:2502`) is only called when a step first suspends with a poll watch. After restart, `_cleanup_zombie_jobs` (`server.py:337`) skips jobs with suspended runs, and `_poll_external_changes` never re-schedules poll timers.
...
This is a known gap documented here for visibility. Fixing it is tracked separately from this test flow.
