...
- **Agent steps now record a token breakdown, not just a dollar figure.** The ACP prompt result has always carried `result.usage` — `inputTokens`, `outputTokens`, `cachedReadTokens`, `cachedWriteTokens`, `totalTokens` — and stepwise discarded it: `extract_cost()` read only `usage_update.cost.amount`, and nothing in the codebase referenced the token fields. New `extract_usage()` / `normalize_usage()` in `acp_ndjson.py` capture them; `AgentStatus.usage` carries them; `executor_meta["usage"]` persists them; `Engine._run_tokens()` / `job_tokens()` aggregate them (including sub-jobs); `status` exposes `tokens` per step and per job.
- **Why this mattered:** under subscription billing `cost_usd` is deliberately zeroed (`AgentExecutor._extract_output` — an agent run carries no dollar charge), so cost-only metering reported `0` for *every* agent step while the real scarce resource — rate-limit quota, denominated in tokens — went unmeasured. Observed in production: virtually every agent step recorded `cost_usd=0` across a week in which an account's weekly window silently filled and killed consecutive scheduled jobs — the outage was legible only after the fact, from the provider's own session logs. Cost answers "what was billed"; usage answers "what was consumed". Both are now kept, and the subscription zeroing applies to dollars only — tokens are never zeroed.
- **`billable_input_tokens`** is derived (fresh input + cache writes, excluding discounted cache reads) because the split is the point: in the captured fixture 94% of input was cache reads, so totals alone overstate quota burn by ~18×.
- **Unmetered renders as absent, never zero.** `extract_usage` returns `None` rather than a zero-filled dict, `_run_tokens` returns `{}`, and the `tokens` key is omitted from status payloads — so "not measured" stays distinguishable from "measured, and it was nothing". Unknown fields in the ACP payload are dropped rather than summed.
- Docs: [`executors.md`](docs/executors.md) gains a Token metering section (plus a note that `max_cost_usd` cannot trigger under subscription billing); [`api.md`](docs/api.md) documents the `tokens` object on `/status` and the always-zero `/cost` under subscriptions.

### Fixed — cancellation provenance & delegated-client reaping
- **SIGTERM'd `--wait` clients no longer cancel server-owned jobs** — a delegated `stepwise run --wait` / `stepwise wait` client that received SIGTERM (parent session reaping background children, timeout wrappers, `./run … stop`) POSTed a job cancel on its way out, destroying server-side work with no recorded cause (observed in production: two scheduled jobs cancelled ~30 s after launch, 0 completions, no recorded cause). SIGTERM now DETACHES the client and leaves the job running (`{"status": "detached"}` on stdout); interactive SIGINT (Ctrl-C) keeps its cancel semantics. Local-engine (non-delegated) wait loops still cancel on any signal — the engine dies with the process.
- **`cancel_job` records who/why** — `Engine.cancel_job(job_id, reason=…, source=…)` now emits a `job.cancelled` event (previously NO event was emitted — an externally cancelled job left zero trace in the event log) and stamps the reason into each cancelled run's error (`"Job cancelled: <reason>"`). `POST /api/jobs/{id}/cancel` accepts an optional `{"reason", "source"}` body (defaults `source="api"`); the CLI cancel command, delegated-client SIGINT handlers, engine reset/fail-fast cascades all pass provenance.
